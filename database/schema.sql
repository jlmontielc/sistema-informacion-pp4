...
CREATE TABLE rutinas_asignadas (
  ...
  duracion_semanas INT,
  observaciones TEXT,
  dias_semana JSON COMMENT 'Mapa de slots a días: {"1":{"diaSemana":1,"nombre":"Lunes"},"2":...}',
  personalizada_por_entrenador BOOLEAN DEFAULT FALSE COMMENT 'TRUE=ajuste manual, FALSE=generada por IA',
  decision ENUM('pendiente', 'aprobada', 'rechazada', 'modificada') NOT NULL DEFAULT 'pendiente' COMMENT 'Estado del flujo HITL',
  fecha_inicio DATE,
  fecha_fin DATE,
  activa BOOLEAN DEFAULT TRUE,
